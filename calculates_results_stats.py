...
def calculates_results_stats(results_dic):
    ...
    results_stats_dic['n_match'] = sum([v[2] for v in results_dic.values()])
    results_stats_dic['n_correct_dogs'] = sum([v[4]*v[3] for v in results_dic.values()])
    results_stats_dic['n_correct_notdogs'] = sum([1 for v in results_dic.values() if (v[4]==0 and v[3]==0)])
    results_stats_dic['n_correct_breed'] = sum([v[2]*v[3] for v in results_dic.values()])
    results_stats_dic['pct_match'] = (100 * results_stats_dic['n_match'] / len(results_dic)) if len(results_dic) > 0 else 0
